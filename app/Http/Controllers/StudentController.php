	...
	*/
    ...
    public function storePreview(Request $request)
    {
        ...
        if ($id) {
            ...
            if ($activity->class_code == 11 || $activity->class_code == 12) {
                if (isset($third_subject) && count($third_subject) > 0 && $fourth_subject && count($fourth_subject) > 0) {
                    if (Auth::user()->group_id == 4) {
                        $this->addsection($request->gender, $third_subject, $fourth_subject, $activity->class_id, $activity->group_id, $student_code, $activity->session_id, $activity->version_id);
                    }
                } else {
                    return redirect()->route('StudentProfile', $request->id)->with('warning', 'Third And Fourth Subject Choose First');
                }
