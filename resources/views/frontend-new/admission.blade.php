                    ...
                    <div class="container spaceb50">
                        <div class="row">
                            ...
                            <div class="refine-categ-header " style="margin-top: 10px;">
                                ...
                                <h3 style="text-align: center">College Admission (কলেজ ভর্তি):
                                    ({{ $sessions->session_name . '-' . ((int) $sessions->session_name + 1) }}) </h3>
                                <form action="{{ route('payment') }}" method="post" enctype="multipart/form-data"
                                    ...
                                    <input type="hidden" name="version_id" id="version_id"
                                        value="{{ $checkadmission->version_id }}" />
                                    <input type="hidden" name="class_id" id="class_id"
                                        value="{{ $checkadmission->class_id }}" />
                                    <input type="hidden" name="session_id" id="session_id"
                                        value="{{ $checkadmission->session_id }}" />
                                    <div class="row">
                                        <div class="col-md-6 col-sm-6">
                                            <div class="form-group">
                                                <label>SSC Roll Number</label><small class="req"> *</small>
                                                <input type="text" readonly="" class="form-control" required=""
                                                    value="{{ $checkadmission->roll_number }}" name="roll_number"
                                        ...
                                        <div class="col-md-6 col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="board_id" id="board_id">
                                                    ...
                                                    <option value="Dhaka"
                                                        {{ strtolower($checkadmission->board) == 'dhaka' ? 'selected="selected"' : '' }}>
                                                        Dhaka (ঢাকা)</option>
                                                    <option value="Rajshahi"
                                                        {{ strtolower($checkadmission->board) == 'rajshahi' ? 'selected="selected"' : '' }}>
                                                        Rajshahi (রাজশাহী)</option>
                                                    <option value="Cumilla"
                                                        {{ strtolower($checkadmission->board) == 'cumilla' ? 'selected="selected"' : '' }}>
                                                        Cumilla (কুমিল্লা)</option>
                                                    <option value="Jashore
"
                                                        {{ strtolower($checkadmission->board) == 'jashore' ? 'selected="selected"' : '' }}>
                                                        Jashore (যশোর)</option>
                                                    <option value="Chattogram
"
                                                        {{ strtolower($checkadmission->board) == 'chattogram' ? 'selected="selected"' : '' }}>
                                                        Chattogram
                                                        (চট্টগ্রাম)</option>
                                                    <option value="Barishal"
                                                        {{ strtolower($checkadmission->board) == 'barishal' ? 'selected="selected"' : '' }}>
                                                        Barishal (বরিশাল)</option>
                                                    <option value="Sylhet"
                                                        {{ strtolower($checkadmission->board) == 'sylhet' ? 'selected="selected"' : '' }}>
                                                        Sylhet (সিলেট)</option>
                                                    <option value="Dinajpur"
                                                        {{ strtolower($checkadmission->board) == 'dinajpur' ? 'selected="selected"' : '' }}>
                                                        Dinajpur (দিনাজপুর)</option>
                                                    <option value="Madrasah"
                                                        {{ strtolower($checkadmission->board) == 'madrasah' ? 'selected="selected"' : '' }}>
                                                        Madrasah (মাদ্রাসা)</option>
                                                    <option
                                                        {{ strtolower($checkadmission->board) == 'mymensingh' ? 'selected="selected"' : '' }}
                                                        value="Mymensingh">Mymensingh (ময়মনসিংহ)</option>
                                                    <option value="BTEB"
                                                        {{ strtolower($checkadmission->board) == 'bteb' ? 'selected="selected"' : '' }}>
                                                        Bangladesh Technical Education (বাংলাদেশ কারিগরি শিক্ষা বোর্ড)
                                                    </option>
                                                    <option value="BOU"
                                                        {{ strtolower($checkadmission->board) == 'bou' ? 'selected="selected"' : '' }}>
                                                        BOU (বাউবি)</option>
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-6 col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" readonly="" class="form-control" required=""
                                                    value="{{ $checkadmission->full_name }}" name="full_name"
                                                    id="full_name" autocomplete="off">
                                        ...
                                        <div class="col-md-6 col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <div><input type="text" readonly="readonly" class="form-control"
                                                        placeholder="Group Name" required=""
                                                        value="{{ $checkadmission->group_name }}" readonly="readonly"
                                                        name="group_name" id="group_name" autocomplete="off">
                                                </div>
                                        ...
                                        <div class="col-md-6 col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" readonly="readonly"
                                                    value="{{ $checkadmission->student_code }}" class="form-control"
                                                    placeholder="Valid Email,phone or username. Username should not contain spaces and spacial character"
                                                    required="" name="username" id="username" autocomplete="off">
                                        ...
                                        <div class="col-md-6 col-sm-6">
                                            <div class="form-group">
                                                <label>Phone (ফোন) (This Number Must be Used While Enrolled In Shaheen
                                                    College Dhaka)</label><small class="req"> *</small>
                                                <input type="number" class="form-control"
                                                    value="{{ $checkadmission->mobile }}"
                                                    placeholder="Valid Phone Number Ex. 01XXXXXXXXX (ফোন)" required=""
