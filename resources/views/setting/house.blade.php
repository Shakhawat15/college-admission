...
@section('content')
    ...
    <style>
        /* ============================================
                   HOUSE PAGE STYLES - VIBRANT TEAL/CYAN EDITION
                   ============================================ */
...
<div class="content-wrapper">
        ...
        <nav aria-label="breadcrumb" class="mb-4 animate-fade-in">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page" style="color: #0891b2; font-weight: 600;">
                    <i class="bx bx-home me-1"></i> House
                </li>
            </ol>
        </nav>
          ...
          <div class="col-md mb-4 mb-md-0 animate-fade-in">
             <div class="card modern-card list-card">
                <h5 class="card-header">
                    <i class="bx bx-home me-1"></i> House List
                    <span class="badge bg-light text-dark ms-2">{{ count($houses) }}</span>
                </h5>
                                ...
                                <tr id="row{{ $house->id }}">
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="avatar avatar-sm" style="width: 32px; height: 32px; background: linear-gradient(135deg, #06b6d4, #0891b2); color: #fff; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 14px;">
                                                <i class="bx bx-home"></i>
                                            </div>
                                            <span class="fw-semibold">{{ $house->house_name }}</span>
                                        </div>
                                    </td>
                                    <td>
                                        <span class="status-badge {{ $house->active == 1 ? 'active' : 'inactive' }}">
                                            <i class="bx {{ $house->active == 1 ? 'bx-check-circle' : 'bx-x-circle' }} me-1"></i>
                                            {{ $house->active == 1 ? 'Active' : 'Inactive' }}
                                        </span>
                                    </td>
                                    @if (Auth::user()->is_view_user == 0)
                                    <td style="text-align: center;">
                                        <div class="d-flex justify-content-center gap-2">
                                            ...
                                            <button class="action-btn delete"
                                                data-url="{{ route('house.destroy', $house->id) }}"
                                                data-id="{{ $house->id }}"
                                                title="Delete">
    ...
    <div class="card modern-card form-card">
        <h5 class="card-header">
            <i class="bx bx-plus-circle me-1"></i> <span id="formTitle">Add New House</span>
        </h5>
        <div class="card-body">
            <form class="needs-validation" method="post" action="{{ route('house.store') }}" novalidate=""
                id="formsubmit">
                @csrf
                <input type="hidden" name="id" id="id" value="0" />

                <div class="mb-3">
                    <label class="form-label-modern" for="house_name">
                        <i class="bx bx-text me-1" style="color: #06b6d4;"></i> House Name
                    </label>
                    <input type="text" class="form-control form-control-modern" name="house_name" id="house_name"
                ...
                <div class="mb-4">
                    <label class="form-label-modern" for="active">
                        <i class="bx bx-toggle-left me-1" style="color: #06b6d4;"></i> Status
                    </label>
                        ...
                        @if (Auth::user()->is_view_user == 0)
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-gradient" id="submit">
                                    <i class="bx bx-save me-1"></i> <span id="submitText">Save House</span>
                                </button>
    ...
    <script>
        ...
        $(function() {
            ...
            $('#resetForm').on('click', function() {
                ...
                $('#submitText').text('Save House');
                $('#formTitle').text('Add New House');
