...
@section('content')
    ...
    <style>
        /* ============================================
                   VERSION PAGE STYLES - VIBRANT COLOR EDITION
                   ============================================ */
...
<div class="content-wrapper">
        ...
        <nav aria-label="breadcrumb" class="mb-4 animate-fade-in">
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    <a href="{{ route('dashboard') }}" style="color: #10b981; text-decoration: none;">
                        <i class="bx bx-home me-1"></i> Dashboard
                    </a>
                </li>
                <li class="breadcrumb-item active" aria-current="page" style="color: #059669; font-weight: 600;">
                    <i class="bx bx-layer me-1"></i> Versions
                </li>
            </ol>
        </nav>
            ...
            <div class="col-xl-8 col-lg-7 animate-fade-in">
                <div class="card modern-card list-card">
                    <div class="card-header">
                        <i class="bx bx-list-ul"></i> Version List
                        <span class="badge bg-light text-dark ms-2">{{ count($versions) }}</span>
                    </div>
                            ...
                            <table class="table table-modern mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr id="row{{ $version->id }}">
                                        <td>
                                            <span class="fw-bold" style="color: #10b981;">{{ $key + 1 }}</span>
                                        </td>
                                        <td>
                                            <div class="d-flex align-items-center gap-2">
                                                <div class="avatar avatar-sm" style="width: 32px; height: 32px; background: linear-gradient(135deg, #10b981, #059669); color: #fff; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 14px;">
                                                    <i class="bx bx-layer"></i>
                                                </div>
                                                <span class="fw-semibold">{{ $version->version_name }}</span>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="badge bg-light text-dark border">{{ $version->version_code }}</span>
                                        </td>
                                        <td>
                                            <span class="status-badge {{ $version->active == 1 ? 'active' : 'inactive' }}">
                                                <i class="bx {{ $version->active == 1 ? 'bx-check-circle' : 'bx-x-circle' }} me-1"></i>
                                        ...
                                        <td style="text-align: center;">
                                            <div class="d-flex justify-content-center gap-2">
                                                ...
                                                <button class="action-btn delete"
                                                    data-url="{{ route('version.destroy', $version->id) }}"
                                                    data-id="{{ $version->id }}"
                                                    title="Delete">
    ...
    <div class="card modern-card form-card">
        <div class="card-header">
            <i class="bx bx-plus-circle"></i> <span id="formTitle">Add New Version</span>
        </div>
        <div class="card-body">
            <form class="needs-validation" method="post" action="{{ route('version.store') }}" novalidate=""
                id="formsubmit">
                @csrf
                <input type="hidden" name="id" id="id" value="0" />

                <div class="mb-3">
                    <label class="form-label-modern" for="version_name">
                        <i class="bx bx-text me-1" style="color: #10b981;"></i> Version Name
                    </label>
                    <input type="text" class="form-control form-control-modern" name="version_name" id="version_name"
                        placeholder="Enter version name (e.g., Version 2024)" required="">
                    <div class="valid-feedback"> Looks good! </div>
                    <div class="invalid-feedback"> Please enter Version Name. </div>
                </div>

                <div class="mb-3">
                    <label class="form-label-modern" for="version_code">
                        <i class="bx bx-hash me-1" style="color: #10b981;"></i> Version Code
                    </label>
                    <input type="text" class="form-control form-control-modern" name="version_code" id="version_code"
                        placeholder="Enter version code (e.g., V2024)" required="">
                    <div class="valid-feedback"> Looks good! </div>
                    <div class="invalid-feedback"> Please enter Version Code. </div>
                </div>

                <div class="mb-4">
                    <label class="form-label-modern" for="active">
                        <i class="bx bx-toggle-left me-1" style="color: #10b981;"></i> Status
                    </label>
                        ...
                        @if (Auth::user()->is_view_user == 0)
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-gradient" id="submit">
                                    <i class="bx bx-save me-1"></i> <span id="submitText">Save Version</span>
                                </button>
    ...
    <script>
        ...
        $(function() {
            ...
            $(document.body).on('click', '.edit', function() {
                var id = $(this).data('id');
                var version_name = $(this).data('version_name');
                var version_code = $(this).data('version_code');
                ...
                $('#submitText').text('Update Version');
                $('#formTitle').text('Edit Version');
            ...
            $('#resetForm').on('click', function() {
                ...
                $('#submitText').text('Save Version');
                $('#formTitle').text('Add New Version');
